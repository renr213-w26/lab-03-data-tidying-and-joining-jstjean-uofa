...
a.  `glimpse()` at the `country_inflation` data frame and answer the following questions based on the output. How many rows does `country_inflation` have and what does each row represent? How many columns does `country_inflation` have and what does each column represent?
    ...
    Comment: It has 44 rows that represent the country. It has 30 columns which represent the various inflation levels as percentages by year.
...
Comment: The 3 countries with the highest inflation rates in 2021 were Argentina (\~48%), Turkiye ( \~20%), and Brazil ( \~8%). These rates are a significant amount higher (almost 10 times higher in the case of Argentina) than the United States 2021 inflation rate which was approximately 5%.
...
Comment: The country with the largest inflation change was New Zealand, it's inflation increased by 3.06 between 1993 and 2021.
...
glimpse(inflation_pivot)
```

Comment: The new data set has 1,276 rows and 3 columns.
...
a.  What is the highest inflation rate observed between 1993 and 2021? The output of the pipeline should be a data frame with one row and three columns. In addition to code and output, your response should include a single sentence stating the country and year.
    ...
    Comment: The highest inflation rate is Brazil in 1994 at 2,075% increase.
...
b.  What is the lowest inflation rate observed between 1993 and 2021? The output of the pipeline should be a data frame with one row and three columns. In addition to code and output, your response should include a single sentence stating the country and year.
    ...
    Comment: The lowest inflation rate is Ireland in 2009 with a 4.48% decrease. (-4.48%)
...
Comment: I chose these 4 countries to represent developed countries on 4 different continents across the world. I would have included an African country if there was one in the data set.
...
inflation_interest <- inflation_pivot |>
...
-   Plot should be customized in at least one way – you could use a different than default color scale, or different than default theme, or some other customization.
    ...
    ```{r}
    ggplot(inflation_interest, aes(x=year, 
    y= annual_inflation, colour = country, shape = country)) +
      geom_line() +
      geom_point(size = 2) +
      scale_colour_manual(
        values = c(
      ...
      labs(
        title = "Annual Inflation Rates for Developed Countries",
        subtitle = "1993-2021",
        x = "Year",
        y = "Inflation (%)",
        colour = "Country",
    ...
    Comment: The patterns of this plot reveal that Australia, Canada and France during most years are experiencing an increase in inflation while Japan is often near 0% or in the negatives (decrease). I am not very familiar with Japan's economy so this is surprising to me. It is not surprising to me that Canada and France follow similar patterns but Australia being often much higher is also a surprise.
...
Comment: It has 132 rows and 4 columns. The variables are country, cpi division id, year, and annual inflation.
...
Comment: it has 12 rows and 2 columns. It has the id number and the description of what each division is responsible for.
...
```{r}
us_inflation_join <- us_inflation |>
  left_join(cpi_divisions, by = c("cpi_division_id" = "id"))
```

```{r}
glimpse(us_inflation_join)
```

Comment: It has 132 rows and 5 columns with the names "country", "cpi_division_id", "year", "annual inflation", and "description".
...
Comment: I chose these 4 because they represent the costs of essentials for the average household.
...
```{r}
us_inflation_interest <- us_inflation_join |>
  filter(description %in% divisions_of_interest)
```

```{r}
us_inflation_interest |>
...
```{r}
us_inflation_interest |>
ggplot(aes(x = year, y = annual_inflation, colour = description, shape = description,)) +
  geom_line() + 
  geom_point(size = 2) +
  scale_colour_manual(
    values = c(
      "Food and non-Alcoholic beverages" = "green",
      "Housing, water, electricity, gas and other fuels" = "brown",
      "Transport" = "red",
      "Communication" = "blue"
    )
  )+ 
  labs(
    title = "U.S Annual Inflation by CPI Division",
    subtitle = "Divisions related to essential household spending",
    x = "Year",
    y = "Inflation (%)",
    colour = "Division",
    shape = "Division"
...
Comment: It appears that for the most part the inflation of Communication, Food and non-Alcoholic beverages and Housing, water, electricity, gas and other fuels are relatively stable. It is surprising to me that Transport is the most variable, jumping up and down and hitting the highest and lowest values and it makes me wonder why that is.
